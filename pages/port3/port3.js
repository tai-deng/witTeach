...
Page({
    ...
    request.request({
      ...
      if (res.address) {
        res.address.forEach(element => {
          address.push(element.province)
        });
      }
      that.setData({data:res,id,address,dir:address[0]})
    })
  },
  binddrawings(e) {
    // 提款
  ...
  bindtotal(e) {
    ...
    this.getData(this.id,month,dir)
  },
  bindaddress(e) {
    // 选择地区
    var index = Number(e.detail.value);
